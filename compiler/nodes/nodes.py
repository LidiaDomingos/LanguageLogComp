...
class BinOp(Node):
    ...
    def Evaluate(self, symbolTable):
        ...
        if (nodeL.Evaluate(symbolTable)[1] == nodeR.Evaluate(symbolTable)[1]):
            ...
            raise TypeError("There is something wrong in players's types!")
...
class Assign(Node):
    ...
    def Evaluate(self, symbolTable):
        ...
        if ((key.Evaluate(symbolTable)[1] == "string" and value.Evaluate(symbolTable)[1] == "int") or (value.Evaluate(symbolTable)[1] == "string" and key.Evaluate(symbolTable)[1] == "int") ):
            raise TypeError(f"When assigned, must be the same type!")
